...
# Logger structuré
logger = setup_logger('bingebear.bot')
...
# Usernames autorisés pour les commandes de streaming (configurable via .env)
...
def get_categories():
    ...
    try:
        ...
        logger.info(f"Categories chargees: {len(categories_cache)}")
...
def get_channels_by_category(category_id):
    """Récupérer les chaînes d'une catégorie (avec retry)"""
    ...
    try:
        ...
        logger.debug(f"Requete chaines categorie {category_id}...")
        response = retry_sync(
            lambda: requests.get(url, headers=HEADERS, timeout=30),
        ...
        logger.info(f"Chaines chargees: {len(channels_list)}")
        return channels_list
    except Exception as e:
        logger.error(f"Erreur chargement chaines: {e}")
        return []
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Commande /start"""
    logger.info(f"Commande /start recue de {update.effective_user.first_name}")
...
async def categories_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("Commande /categories recue")
...
async def cat_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("Commande /cat recue")
...
async def play_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info("Commande /play recue")
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Arrêter le stream"""
    global current_stream, pytgcalls
    logger.info("Commande /stop recue")
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Statut du stream"""
    logger.info("Commande /status recue")
    if current_stream:
...
async def test_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    global current_stream, pytgcalls
    logger.info("Commande /test recue")
...
async def setiptv_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.exception(f"Erreur dans setiptv_command: {e}")
        await update.message.reply_text(f"❌ Erreur de connexion: {e}")


async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Aide"""
    logger.info("Commande /help recue")
    help_text = (
...
async def post_init(application):
    ...
    logger.info(f"Connecte en tant que: {me.first_name} (@{me.username})")
...
def main():
    """Fonction principale"""
    logger.info("BingeBear TV - Live Streaming Bot")
    logger.info("=" * 40)
    ...
    logger.info("Bot pret! Envoyez /start a @Bingebear_tv_bot")
